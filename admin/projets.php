...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['new','edit'])) {
  ...
  if (!$titre || !$description) {
    ...
    if ($action === 'new') {
      $stmt = $pdo->prepare('INSERT INTO projets (titre,description,type,url_demo,url_download,visible,ordre,created_at) VALUES (?,?,?,?,?,?,?,NOW())');
      $stmt->execute([$titre,$description,$type,$url_demo,$url_dl,$visible,$ordre]);
      $new_id = (int)$pdo->lastInsertId();
      foreach ($techs_sel as $tid) { $pdo->prepare('INSERT IGNORE INTO projet_tech(projet_id,tech_id) VALUES(?,?)')->execute([$new_id,$tid]); }
    } else {
      $pdo->prepare('UPDATE projets SET titre=?,description=?,type=?,url_demo=?,url_download=?,visible=?,ordre=? WHERE id=?')->execute([$titre,$description,$type,$url_demo,$url_dl,$visible,$ordre,$id]);
      $pdo->prepare('DELETE FROM projet_tech WHERE projet_id = ?')->execute([$id]);
      foreach ($techs_sel as $tid) { $pdo->prepare('INSERT INTO projet_tech(projet_id,tech_id) VALUES(?,?)')->execute([$id,$tid]); }
    }
      ...
      <form method="POST" class="admin-form">
        <div class="form-row">
          <div class="field">
            ...
            <input type="text" name="titre" required maxlength="150" value="<?= e($projet['titre'] ?? '') ?>" placeholder="Nom du projet">
          </div>
          <div class="field">
            <label>Type <span class="req">*</span></label>
            <select name="type">
              <?php foreach (['site' => 'Site web','app' => 'Application','jeu' => 'Jeu'] as $val => $lbl): ?>
                <option value="<?= $val ?>" <?= ($projet['type'] ?? '') === $val ? 'selected' : '' ?>><?= $lbl ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="field">
          ...
          <textarea name="description" required rows="7" placeholder="Décrivez le projet en détail..."><?= e($projet['description'] ?? '') ?></textarea>
        </div>

        <div class="form-row">
          <div class="field">
            <label>URL Demo / Visite</label>
            <input type="url" name="url_demo" placeholder="https://…" value="<?= e($projet['url_demo'] ?? '') ?>">
          </div>
          <div class="field">
            <label>URL Téléchargement</label>
            <input type="url" name="url_download" placeholder="https://…" value="<?= e($projet['url_download'] ?? '') ?>">
          </div>
        </div>
          ...
          <div class="techs-checkboxes">
            <?php foreach ($toutes_techs as $t): ?>
              <label class="tech-check">
                <input type="checkbox" name="techs[]" value="<?= (int)$t['id'] ?>" <?= in_array($t['id'], $proj_techs ?? []) ? 'checked' : '' ?>>
                <span class="tech-check-label" style="--tc:<?= e($t['couleur']) ?>"><?= e($t['nom']) ?></span>
              </label>
            <?php endforeach; ?>
          </div>
        </div>
        ...
        <div class="form-actions">
          <button type="submit" class="btn btn-primary"><?= $action === 'new' ? '✓ Créer le projet' : '✓ Enregistrer' ?></button>
      ...
      <?php else: ?>
        <div class="admin-table-wrap">
          <table class="admin-table">
            <thead>
              <tr><th>Titre</th><th>Type</th><th>Technologies</th><th>Ordre</th><th>Visible</th><th>Actions</th></tr>
            </thead>
            <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <div class="tags-wrap">
                      ...
                      <?php endforeach; ?>
                      <?php if (empty($p['techs'])): ?><span style="color:var(--text-3);font-size:11px">—</span><?php endif; ?>
                    </div>
                  </td>
                  <td><?= (int)$p['ordre'] ?></td>
                  <td>
                    <a href="<?= SITE_URL ?>/admin/projets.php?action=toggle&id=<?= (int)$p['id'] ?>" style="cursor:pointer">
                      <?= $p['visible'] ? '<span class="badge badge-ok">Oui</span>' : '<span class="badge badge-off">Non</span>' ?>
                    </a>
                  </td>
                  <td class="td-actions">
